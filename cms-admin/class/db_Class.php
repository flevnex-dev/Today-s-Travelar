...
class db_class {
    ...
    public function pdocon() {
        $db=new PDO("mysql:host=localhost;dbname=travel_info;", "root", "");
    ...
    function baseUrl($suffix='') {
        ...
        $web_root=$protocol . $_SERVER['HTTP_HOST'] . "/" . "a.m.softltd/Today's Travelar/Today's Travelar/cms-admin/";
        $suffix=ltrim($suffix, '/');
    ...
    function LbaseUrl($suffix='') {
        $protocol=strpos($_SERVER['SERVER_SIGNATURE'], '443') !== false ? 'https://' : 'http://';
        $web_root=$protocol . $_SERVER['HTTP_HOST'] . "/" . "a.m.softltd/Today's Travelar/Today's Travelar/";
        $suffix=ltrim($suffix, '/');
        return $web_root . trim($suffix);
    }
