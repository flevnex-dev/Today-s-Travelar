<?php 
		...
		$table="right_bottom_ad"; ?>
		<?php 
		if(isset($_POST['create'])){
			extract($_POST);
			if(!empty($_FILES['image']['name']) && !empty($priority))
			{ include('class/uploadImage_Class.php'); $imgclassget=new image_class(); 
			$image=$imgclassget->upload_fiximage("upload","image","image_upload_".$table_name."_".time());  $insert=array('image'=>$image,'priority'=>$priority,'date'=>date('Y-m-d'),'status'=>1);
		...
		{
			extract($_POST);if(!empty($priority))
			{$updatearray=array("id"=>$id);if(!empty($_FILES['image']['name']))
					{
						include('class/uploadImage_Class.php'); $imgclassget=new image_class(); $image_1=$imgclassget->upload_fiximage("upload","image","image_upload_".$table_name."_".time());
						$image=$image_1;
						@unlink("upload/".$ex_image);
					}else{
						$image=$ex_image;
					}$upd2=array('image'=>$image,'priority'=>$priority,'date'=>date('Y-m-d'),'status'=>1);
...
<!--[if !IE]><!--><html><!-- <![endif]-->
    ...
    <body class="">
        ...
        <div id="content">
        	<h1 class="content-heading bg-white border-bottom">Right bottom Ad</h1>
            <div class="innerAll bg-white border-bottom">
                <ul class="menubar">
                    <li class="active"><a href="#">Create New Right bottom Ad</a></li>
                    <li><a href="right_bottom_ad_data.php">Right bottom Ad List</a></li>
                </ul>
            </div>
                ...
                <!-- Widget -->
                        ...
                        <div class="widget widget-inverse" >
                            ...
                            <div class="widget-head">
                                <h4 class="heading">Update/Change - Right bottom Ad</h4>
                            </div>
                            <!-- // Widget heading END -->
                            ...
                            <!-- Widget heading -->
                            <div class="widget-head">
                                <h4 class="heading">Create New Right bottom Ad</h4>
                            </div>
                            <!-- // Widget heading END -->
